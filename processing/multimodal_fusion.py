...
class MultimodalFusionEngine:
    ...
    def _get_fallback_animation(self, audio_event: Dict, effect_decision: Dict) -> str:
        ...
        if energy > 0.7 or tier == 'major': return AnimationType.SHAKE
        if onset_type == 'HIGH_FREQ': return AnimationType.POP_SHRINK
        if onset_type == 'LOW_FREQ': return AnimationType.PULSE
        if onset_type == 'BROADBAND': return AnimationType.EXPLODE_OUT
        if energy > 0.4: return AnimationType.WIGGLE
    ...
    def process_multimodal_events(
        ...
        sync_offset: float = -0.15 # Default, can be overridden
    ) -> List[Dict]:
        self.log_func(
            f"🔄 Fusing {len(audio_events)} audio events with {len(video_analyses_map)} video analyses (Sync: {sync_offset}s)..."
        )
            ...
            if matching_video:
                ...
                if should_generate:
                    ...
                    final_effects.append(
                        self._create_final_effect(effect_decision, audio_event, chosen_animation, sync_offset)
                    )
    ...
    def _find_closest_video_analysis(
        self, audio_event: Dict, video_analyses_map: Dict[float, Dict], max_time_diff: float = 1.0,
    ) -> Dict:
    ...
    def _make_fusion_decision(
        ...
        speech_indicators = ["talking", "speaking", "says", "conversation", "dialogue", "reacts"]
        strong_action_indicators = ["shoot", "shot", "gun", "fire", "punch", "hit", "kick", "crash", "explode"]
    ...
    def _fallback_audio_effect(self, audio_event: Dict) -> str:
        energy = audio_event.get('energy', 0.5)
        onset_type = audio_event.get('onset_type', 'GENERAL')
        if onset_type == 'LOW_FREQ': return "BOOM" if energy > 0.6 else "THUD"
        elif onset_type == 'HIGH_FREQ': return "CRACK" if energy > 0.6 else "CLICK"
        elif onset_type == 'BROADBAND': return "CRASH" if energy > 0.6 else "BUMP"
        else:
    ...
    def _create_final_effect(
        self, decision: Dict, audio_event: Dict, animation_type: str, sync_offset: float
    ) -> Dict:
        """
        Creates the final event, applying user-defined sync and animation timing.
        """
        precise_peak_time = audio_event.get('peak_time', audio_event['time'])
        
        # 1. Base audio offset (User defined)
        ...
        total_offset = sync_offset + animation_offset
